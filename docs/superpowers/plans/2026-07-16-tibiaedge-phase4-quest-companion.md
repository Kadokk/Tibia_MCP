...
- **Plan-bug ruling — Task 15 section order (2026-07-16):** the Spec paragraph (§1575) stated the final render order as card → facts → goals → **gists → quests**, while Step 3's implementation note (§1604) said card → facts → goals → **quests → gists** — a plan-text bug from folding in a reviewer note where the intro prose got updated but the Step 3 parenthetical was missed. Ruling (Brain): Option X, quests before gists, per Step 3 — tests are indifferent to the order, the v2 spec groups quests with goals, and it's the minimal diff (no restructuring of the existing premium block). Implemented as such in commit for Task 15.
