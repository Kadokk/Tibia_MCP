...
- **2026-07-21, Task 12 follow-up (orchestrator-directed, interstitial):** rendering real smoke-DB rows through the new catalog tools surfaced a real defect — raw `[[link]]` wiki markup was not stripped to display text in creature ability/loot/location values (only `{{template}}` markup had a unified guard, from Task 6). Directed as a small fix before Task 14's eval work, so eval cases don't bake in dirty strings: strip `[[...]]` to display text across the affected mappers, extend the no-markup test to assert `[[` as well as `{{`, and add a `--force` CLI flag to backfill existing dirty rows (the revid gate means a parser fix alone does not heal already-imported rows). Separately, Coder flagged a lower-severity, in-spec limitation (incidental `[[level]]` link in creature location prose extracted as a place name) — left alone per its own Task 5 assessment, not a new defect.
