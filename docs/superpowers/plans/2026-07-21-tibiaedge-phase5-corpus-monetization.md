...
- **2026-07-21, three findings surfaced during atomicity verification, triaged.** (1) `--force` clears `source_revision` for the WHOLE content type, not scoped to `--limit` — the Orchestrator's own `--force --type item --limit 20` verification (and an earlier Coder test) reset all ~6,557 item rows' revision markers on the smoke DB. Not data loss (titles/stats/offers intact, confirmed), self-heals on the next full item run; directed to fix `--force` to respect `--limit` as a quick bundle-in. (2) Brain's production-scale runs (9,972 pages) show the SAME single page failing on every run (runs 22 and 27), unrelated to Naga Nest or any fix landed today — flagged to Brain directly, since it's their run to dig into. (3) Observability gap: per-page failures increment a counter and log to stderr only — nothing durable records which title failed, so "1 failed" out of 9,972 is not diagnosable without re-instrumenting. Deferred as a follow-up (same shape as the eval harness's earlier missing-tool-trace gap); not blocking.
