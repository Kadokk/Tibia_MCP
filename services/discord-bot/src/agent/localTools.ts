...
export const localToolDefs: McpToolDef[] = [
  {
    name: 'remember',
    description:
      'Store one long-term fact about the player, only when they explicitly ask you to remember something (a preference, goal, or piece of context). Phrase it as a short third-person declarative statement. Always call this tool on that trigger: never judge first whether the fact is worth storing, and never decide in advance that it will not be stored. Whether it is available to them is decided server-side and reported in this tool\'s reply, so never skip the call or state the outcome yourself.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'recall_memory',
    description:
      "Search the player's stored long-term memory. Use it whenever the player asks what you remember or know about them, or refers back to preferences, goals or context they gave you earlier. Always call it on that trigger: never judge first whether anything is stored, whether the reply would help, or whether the PLAYER NOTES block already covers it — an absent or empty PLAYER NOTES block says nothing about what is stored. Whether it is available to them is decided server-side and reported in this tool's reply, so never skip the call or state the outcome yourself.",
    inputSchema: {
      type: 'object',
  ...
  },
  {
    name: 'find_items',
    description:
      'List catalog items by any combination of name fragment, object class, equipment slot and maximum level requirement — at least one. Use whenever the player is browsing or comparing rather than asking about one named item: "what armour can I wear at level 40", "what should I buy next". CALL THIS FIRST, WITHOUT ASKING THE PLAYER ANYTHING: turn whatever the question already implies into filters — "body armour" means object_class "Body Equipment" or slot "Body" — show what comes back, and only then offer to narrow. Asking for their level or vocation before showing any results is wrong; clarifying questions come after the list, never instead of it. One filter is enough — if all you can infer is the object class, always call with that class alone; not knowing their level is never a reason to ask instead of calling. For a whole category filter on object_class or slot and leave search empty; putting the category into search matches item NAMES and finds nothing. Use get_item_info instead only when they named a specific item.',
    inputSchema: {
      type: 'object',
      properties: {
