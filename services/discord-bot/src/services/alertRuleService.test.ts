...
describe('AlertRuleService', () => {
  ...
  it('rejects blank inputs and non-positive prices', async () => {
    const repo = { countActiveRules: vi.fn(), createRule: vi.fn() };
    const service = new AlertRuleService(repo, new AccessLimitsService());

    await expect(service.createItemPriceAlert({
      tier: 'free', ownerType: 'user', ownerId: '1', world: '', item: 'gold token', condition: 'below', priceGold: 45000, delivery: 'channel'
    })).rejects.toThrow(/world is required/);
    await expect(service.createItemPriceAlert({
      tier: 'free', ownerType: 'user', ownerId: '1', world: 'Antica', item: ' ', condition: 'below', priceGold: 45000, delivery: 'channel'
    })).rejects.toThrow(/item is required/);
    await expect(service.createItemPriceAlert({
      tier: 'free', ownerType: 'user', ownerId: '1', world: 'Antica', item: 'gold token', condition: 'below', priceGold: 0, delivery: 'channel'
    })).rejects.toThrow(/priceGold must be positive/);
    expect(repo.countActiveRules).not.toHaveBeenCalled();
  });
});
