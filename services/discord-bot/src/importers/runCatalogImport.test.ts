...
describe('parseCatalogImportArgs', () => {
  it('imports every content type when no --type is given', () => {
    expect(parseCatalogImportArgs([])).toEqual({ types: [...CATALOG_IMPORT_ORDER] });
  });

  it('imports only the requested type', () => {
    expect(parseCatalogImportArgs(['--type', 'spell'])).toEqual({ types: ['spell'] });
  });
  ...
  it('reads a page limit', () => {
    expect(parseCatalogImportArgs(['--type', 'spell', '--limit', '3'])).toEqual({ types: ['spell'], limit: 3 });
  });
